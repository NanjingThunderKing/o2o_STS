			...
			try {
				...
				if (userProductMap.getPoint() != null && userProductMap.getPoint() > 0) {
					...
					if (userShopMap != null && userShopMap.getUserShopId() != null) {
						...
						if (effectedNum <= 0) {
							throw new UserProductMapOperationException("更新店铺积分失败");
						}
					} else {
						// 若之前没在店铺消费过，添加一条店铺积分信息(就和会员初始化一样)
						...
						if (effectedNum <= 0) {
							throw new UserProductMapOperationException("创建店铺积分失败");
						}
					}
				}
